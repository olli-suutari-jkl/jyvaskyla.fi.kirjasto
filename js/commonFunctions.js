		...
		while (reFindLinksExec != null) {
			...
			for (var i = 0; i < serviceNamesWithLinks.length; i++) {
				...
				if (textInside.indexOf(escapedName) > -1) {
					var linkToService = reFindLinksExec[0].replace(
						'<p>',
						'<a class="service-link-in-modal" data-name="' + serviceNamesWithLinks[i] + '" href="javascript:void(0);">'
					);
					if (!isModal) {
						linkToService = reFindLinksExec[0].replace(
							'<p>',
							'<a class="service-link-in-description" data-name="' +
								serviceNamesWithLinks[i] +
			...
			if (!matchFound) {
				for (var i = 0; i < libListMultiLang.length; i++) {
					if (
						textInside.indexOf(libListMultiLang[i].nameFi.replace('-', ' ')) > -1 ||
						...
						var linkToService = reFindLinksExec[0].replace(
							'<p>',
							'<a class="library-link-in-description" data-lib="' +
								libListMultiLang[i].id +
								'" href="javascript:void(0);">'
						);
						if (isModal) {
							linkToService = reFindLinksExec[0].replace(
...
function adjustParentUrl(toAdd, type) {
	...
	if (type === 'search') {
		if (refUrl.indexOf('?haku=') > -1) {
			refUrl = refUrl.replace(new RegExp(/(\?haku=.*\*)/, 'i'), '');
		}
		if (isValue(toAdd)) {
	...
	if (type === 'library') {
		for (var i = 0; i < libListMultiLang.length; i++) {
			...
			if (refUrl.indexOf(nameFi) > -1) {
				refUrl = refUrl.replace(new RegExp(nameFi, 'i'), '');
			} else if (refUrl.indexOf(nameEn) > -1) {
				refUrl = refUrl.replace(new RegExp(nameEn, 'i'), '');
			}
		}
	}
