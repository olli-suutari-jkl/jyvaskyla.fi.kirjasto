...
function generateScheduleInfo(data) {
	...
	if (isWeekInfo) {
		...
		$('#scheduleInfo').replaceWith(
			'<span id="scheduleInfo" class="info-span-lg info-text"><i class="fas fa-info-circle" > </i> ' +
				genericDescription +
				'</span>'
		);
	...
	if (isSpecialInfo) {
		...
		$('#specialInfo').replaceWith(
			'<span id="specialInfo" class="info-span-lg info-text"><i class="fas fa-info-circle" > </i>' +
				holidayDescription +
				'</span>'
		);
			...
			if (data.items.length === 0) {
				...
				if (weekNumber <= 0) {
					...
					$('#scheduleInfos').replaceWith(
						'<tbody id="scheduleInfos" class="schedule-infos">' +
				...
				for (var i = 0; i < schedules.length; i++) {
					...
					if (schedules != null) {
						...
						if (schedules[i].info !== null) {
							...
							dayInfo =
								'<tr class="info-row time--sub isTodayClass">' +
								'<td colspan="3"><span class="info-span info-text"><i class="fas fa-info-circle" > </i> ' +
								infoText +
								'</span></td>' +
								'</tr>';
                        ...
                        */
							...
							if (time.status === 1) {
								...
								staffToday =
									...
									'<td class="trn"><i class="fas fa-level-up-alt fa-rotate-90"></i> ' +
							...
							// self-service
								...
								if (staffPresentStart === '') {
									selfServiceBefore =
										...
										'<td><i class="fas fa-level-up-alt fa-rotate-90"></i> ' +
					...
					if (isClosed) {
						...
						scheludeRow =
							...
							isTodayClass +
							'">' +
							'<th class="date-container" scope="row" rowspan="' +
							closedRowSpan +
							'">' +
							'<time datetime="' +
						...
						scheludeRow =
							...
							dayName +
							'</td>' +
							'<td class="main-schedule"><time datetime="' +
							dayStart +
							'">' +
							dayStart.replace(/:/g, '.') +
							'</time> – <time datetime="' +
