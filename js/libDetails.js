// Variables
var withParams =
	'&with=pictures,services,buildingInfo,departments,mailAddress,links,phoneNumbers,primaryContactInfo,' +
...
function asyncFetchServiceNamesInOppositeLang() {
	...
	setTimeout(function () {
		// Disable caching: https://stackoverflow.com/questions/13391563/how-to-set-cache-false-for-getjson-in-jquery
		$.getJSON(
			'https://api.kirjastot.fi/v4/library/' + library + '?lang=' + oppositeLang + '&with=services,departments',
			{ _: new Date().getTime() },
			function (data) {
	...
	setTimeout(function () {
		...
		if (transitIsEmpty) {
			...
			if (coordinates != null && address.street != null && cityName != null) {
				...
				var linkToTransitInfo = address.street + ', ' + address.city + '::' + coordinates.lat + ', ' + coordinates.lon;
	...
	$('.index-item').on('click', function (e) {
		...
		if (popupText.indexOf('<h') !== -1) {
			$('#modalTitle').replaceWith(
				'<h1 id="modalTitle" class="modal-title underlined-title">' + $(this).data('name') + '</h1>'
			);
		} else {
			$('#modalTitle').replaceWith(
				'<h1 id="modalTitle" class="modal-title modal-title-small underlined-title">' + $(this).data('name') + '</h1>'
			);
		}
...
function asyncFetchServices() {
	...
	setTimeout(function () {
		...
		for (var i = 0; i < arrayOfServices.length; i++) {
			// Collections
			if (
				(arrayOfServices[i].name != null && arrayOfServices[i].name.length != 0) ||
				...
				// Services
					...
					if (
						arrayOfServices[i].name === 'Saavutettavuus' ||
						arrayOfServices[i].name === 'Esteettömyyspalvelut' ||
						arrayOfServices[i].name === 'Accessibility services'
					) {
		...
		if (accessibilityCount != 0 || !accessibilityIsEmpty) {
			...
			if (accessibilityCount !== 0) {
				...
				setTimeout(function () {
					$('[data-toggle="accessibility-tooltip"]')
						.popover({
							trigger: 'manual',
							html: true,
							animation: true
			...
			if (counter === data.length) {
				if (igImages.length !== 0) {
					...
					for (var i = 0; i < igImages.length; i++) {
						...
						var igLogo =
							'<a title="' +
							i18n.get('Open in instagram') +
							'" data-placement="bottom" ' +
							'data-toggle="navigation-tooltip" class="slider-ig-logo no-external-icon"' +
							igHref +
							'>' +
						...
						if (igImages[i].type == 'video') {
							...
							vidItem =
								...
								'<video id="sliderVid' +
								i +
								'" class="ig-img ig-vid stopped" poster="' +
								igImages[i].poster +
								'">' +
								'<source src="' +
								igImages[i].url +
								...
								'<input type="range" class="volume-bar" id="volumeBar' +
								[i] +
	...
	setTimeout(function () {
		if (isEmpty($('#buildingDetails')) && !isReFetching) {
			...
			if (founded != null) {
				triviaIsEmpty = false;
				$('#triviaBody').append(
					'<tr><td class="trivia-cell-title"><strong>' +
						i18n.get('Founded') +
						': </strong></td>' +
						'<td class="trivia-detail">' +
						founded +
						'</td></tr>'
				);
			}
...
function asyncFetchImages() {
	...
	setTimeout(function () {
		...
		$.when(generateImages(pictures)).then(function () {
			noImages = false;
			setTimeout(function () {
				...
				if (mailAddress != null && mailAddress.area != null) {
					...
					if (postalString !== libName + '<br>') {
						$('#postalAddress').append(
							'<p><strong>' + i18n.get('Postal address') + '</strong><br>' + postalString + '</p>'
						);
					}
				} else {
				...
				if (libraryList.length !== 0) {
					...
					for (var i = 0; i < libraryList.length; i++) {
						var text =
							...
							libraryList[i].text +
							'</strong><br>' +
							libraryList[i].street +
							', <br>' +
							libraryList[i].zipcode +
							', ' +
							libraryList[i].city +
							'<br><button type="button" value="' +
							libraryList[i].id +
							'" class="map-library-changer btn btn-md btn-primary py-0 mb-3">' +
							i18n.get('Fetch details') +
							'</button>';
						if (libraryList[i].id == library) {
							...
							if (libraryList[i].coordinates === null && !coordinateErrorSet) {
								$('#mapContainer').append(
									'<div id="noCoordinates">' +
										i18n.get('Note') +
										'! ' +
										libraryList[i].text.toString() +
										i18n.get('No coordinates') +
										'</div>'
								);
						...
						if (libraryList[i].coordinates != null) {
							L.marker([libraryList[i].coordinates.lat, libraryList[i].coordinates.lon], { icon: markerIcon })
								.addTo(map)
								.bindPopup(text)
								.openPopup();
						}
					...
					L.marker([lat, lon], { icon: markerIcon }).addTo(map).bindPopup(document.title).openPopup();
					addCoordinatesDeferred.resolve();
				}
			}, 1);
			// Return the Promise so caller can't change the Deferred
...
function generateFbWidgets() {
	...
	if (fbPageNames.length == 1) {
		...
		fbHTML =
			'<div class="fb-page" style="width: ' +
			fbWidth +
			'; margin-bottom: 2em;" data-href="https://www.facebook.com/' +
			fbPageNames[0] +
			'" data-tabs="' +
			tabs +
			'" data-width="' +
		...
		for (var i = 0; i < fbPageNames.length; i++) {
			...
			if (i == 0) {
				feedOne =
					...
					'px" data-height="' +
					descriptionHeight +
					'px" data-small-header="true" data-adapt-container-width="' +
					adaptWidth +
		...
		fbHTML = '<div>' + feedOne + feedTwo + feedThree + feedFour + '</div>';
		$('.facebook-section').append(fbHTML);
		//$('.news-description').after(fbHTML);
	}
...
function generateIGCaption(caption) {
	...
	while (reFindTagsExec != null) {
		...
		tagLink =
			'<a target="_blank" class="external-link" href="https://www.instagram.com/explore/tags/' +
			tagLink +
			'/">' +
			tagText +
			'</a> ';
		tagsToReplace.push({ position: reFindTagsExec[0], replacement: tagLink, type: 'tag' });
	...
	while (reFindUsersExec != null) {
		...
		var tagLink = tagText.substring(1);
		tagLink =
			'<a target="_blank" class="external-link" href="https://www.instagram.com/' + tagLink + '/">' + tagText + '</a> ';
		tagsToReplace.push({ position: reFindUsersExec[0], replacement: tagLink, type: 'user' });
	...
	for (var t = 0; t < tagsToReplace.length; t++) {
		...
		if (tagsToReplace[t].type == 'tag') {
			caption = caption.replace(tagsToReplace[t].position, tagsToReplace[t].replacement.replace('#', '%%%'));
		} else {
			caption = caption.replace(tagsToReplace[t].position, tagsToReplace[t].replacement.replace('@', '<<><<>'));
		}
	}
	...
	setTimeout(function () {
		...
		links.forEach(function (element) {
			...
			if (url.indexOf('facebook') !== -1) {
				...
				if (
					refUrl.length === 0 ||
					url.indexOf(refUrl.replace(/^(?:https?:\/\/)?(?:www\.)?/i, '').split('/')[0]) === -1
				) {
					// Remove httml & www
					...
					if (!checkIfContactExists(contactlist, prettyUrl) && !checkIfNameExists(contactlist, element.name)) {
						contactlist.unshift({
							name: capitalize(sanitizedHTMLString(element.name)),
		...
		if (phoneNumbers.length !== 0) {
			for (var i = 0; i < phoneNumbers.length; i++) {
				...
				if (!checkIfContactExists(numbersList, phoneNumbers[i].number)) {
					...
					numbersList.push({ name: phoneNumbers[i].name, contact: phoneNumbers[i].number + description });
				}
				counter = counter + 1;
			}
...
function generateContacts() {
	...
	setTimeout(function () {
		if (isEmpty($('#contactsTbody'))) {
			$.when(asyncGenerateNumbers(), asyncGenerateStaff()).then(function () {
				...
				for (var i = 0; i < contactlist.length; i++) {
					...
					$('#contactsTbody').append(
						'<tr>' + '<td>' + contactlist[i].name + '</td>' + '<td>' + contactDetail + '</td>' + '</tr>'
					);
				}
				// Phones
				for (var i = 0; i < numbersList.length; i++) {
					...
					$('#contactsTbody').append(
						'<tr>' + '<td>' + numbersList[i].name + '</td>' + '<td>' + contactDetail + '</td>' + '</tr>'
					);
				}
				// Staff
				for (var i = 0; i < staffList.length; i++) {
					...
					$('#contactsTbody').append(
						'<tr>' + '<td>' + staffList[i].name + '</td>' + '<td>' + contactDetail + '</td>' + '</tr>'
					);
				}
				// Show navigation if content.
				if (!isEmpty($('#contactsTbody'))) {
...
function fetchInformation(language, lib) {
	...
	function triggerFetch() {
		...
		setTimeout(function () {
			if (!isReFetching) {
				...
				$.when(asyncFetchV4Data()).then(function () {
					$.when(asyncFetchServiceNamesInOppositeLang()).then(function () {
						$.when(
							asyncFetchDepartments(),
							asyncFetchServices(),
							asyncGenerateTrivia(),
							asyncFetchLinks(),
							asyncFetchLocation()
				...
				$.when(asyncGenerateGenericDetails()).then(function () {
					fetchDeferred.resolve();
				});
			}
		}, 1);
	...
	$.when(triggerFetch()).then(function () {
		...
		if (language == 'en') {
			setTimeout(function () {
				...
				$('header').append(
					'<span class="en-notification">Note: If information is missing in English, Finnish version is used where available.</span>'
				);
			}, 400);
		} else {
			if (language === 'fi') {
				...
				if (noServices) {
					...
					if (descriptionIsEmpty) {
						...
						if (slogan !== null) {
							if (!slogan.length < 2) {
								$('#introductionSidebar').prepend(
									' <blockquote class="blockquote library-slogan">' + slogan + '</blockquote>'
								);
							}
						}
					}
				...
				if (noLeftCol && noSidebar) {
					$('#introductionSidebar').append(
						'<div id="noIntroContent"><h3>' + i18n.get('No content') + ' <i class="fas fa-frown-o"></i></h3></div>'
					);
				}
				...
				$().ready(function () {
					...
					$('#introContent, #accessibilityDetails, table td')
						.filter(function () {
