...
function generateEventListForLib(library) {
	...
	if (eventList.length > 5) {
		$('#keskiEventsUl').append(
			'<li class="event-li load-more-events">' +
				'<button class="show-all-events-btn btn-secondary">' +
				i18n.get('Show all events') +
				'</button>' +
				'</li>'
		);
	...
	$('.event-item-link').on('click', function (e) {
		...
		var itemLocation =
			'<p class="event-detail event-location" aria-label="' +
			i18n.get('Event location') +
			'">' +
			'<img data-toggle="tooltip" title="' +
			i18n.get('Event location') +
			'" data-placement="top" alt="" ' +
			'src="' +
			faPath +
			'map-marker.svg" class="fa-svg event-details-icon">' +
			locationText +
			'</p>';
...
function generateEventItem(event, id) {
	...
	if (event.tags.length !== 0) {
		...
		tagDisplay =
			'<span class="event-detail event-tags" aria-label="' +
			i18n.get('Event category') +
			'">' +
			'<img data-toggle="tooltip" title="' +
			i18n.get('Event category') +
			'" data-placement="top" alt="" ' +
			'src="' +
			faPath +
			'tags.svg" class="fa-svg event-details-icon">' +
			tagDisplay +
			'</span>';
	} // 10 first chars = date.
	...
	if (startDateTime !== '') {
		...
		startTimeDisplay =
			'<img alt="" src="' +
			faPath +
			'calendar.svg" class="fa-svg event-details-icon">' +
			startDateDay +
			'<img alt="" src="' +
			faPath +
			'clock.svg" class="fa-svg event-details-icon event-li-clock">' +
			startDateTime;
	} else {
	...
	if (endDateDay !== '') {
		...
		if (endDateDay == startDateDay) {
			...
			if (endDateTime !== '') {
				endTimeDisplay =
					'<img alt="" src="' + faPath + 'clock.svg" class="fa-svg event-details-icon event-li-clock">' + endDateTime;
				endTimeRowSplit = '<span class="time-row-splitter" aria-hidden="true"> </span>';
			} // End date

			endDateTimeDisplay =
				'<i class="start-end-divider">–</i>' +
				endTimeRowSplit +
				'<img alt="" src="' +
				faPath +
				'calendar.svg" class="fa-svg event-details-icon"> ' +
	...
	if (event.price != '') {
		...
		eventPrice =
			'<span class="event-detail event-price" aria-label="' +
			i18n.get('Price') +
			'">' +
			'<img data-toggle="tooltip" title="' +
			i18n.get('Price') +
			...
			'src="' +
			faPath +
			'money-bill-alt.svg" class="fa-svg event-details-icon">' +
			eventPrice +
			'</span>';
	}
	// Website
	...
	if (event.link_url !== null && event.link_url != '') {
		...
		itemLink =
			'<span class="event-detail event-link" aria-label="' +
			i18n.get('Website') +
			'">' +
			'<img data-toggle="tooltip" title="' +
			i18n.get('Website') +
			...
			'src="' +
			faPath +
			'globe.svg" class="fa-svg event-details-icon"><a href="' +
			event.link_url +
			'">' +
			prettyUrl +
			'</a></span>';
	...
	var itemLocation =
		'<span class="event-detail event-location" aria-label="' +
		i18n.get('Event location') +
		'">' +
		'<img data-toggle="tooltip" title="' +
		i18n.get('Event location') +
		...
		'src="' +
		faPath +
		'map-marker-alt.svg" class="fa-svg event-details-icon">' +
		eventLocation +
		'</span>';
	var locationInfo = '';
	...
	if (locationHelpText !== '') {
		locationInfo =
			'<span class="event-detail event-location-info" aria-label="' +
			i18n.get('Location information') +
			'">' +
			'<img data-toggle="tooltip" title="' +
			i18n.get('Location information') +
			'" data-placement="top" alt="" ' +
			'src="' +
			faPath +
			'location-arrow.svg" class="fa-svg event-details-icon">' +
			locationHelpText +
			'</span>';
	}
	...
	if (linkToNavigation != '') {
		linkToNavigation =
			'<span class="event-detail event-transit" aria-label="' +
			i18n.get('Navigation to location') +
			'">' +
			'<img data-toggle="tooltip" title="' +
			i18n.get('Navigation to location') +
			'" data-placement="top" alt="" ' +
			'src="' +
			faPath +
			'directions.svg" class="fa-svg event-details-icon">' +
			linkToNavigation +
			'</span>';
	}
	...
	var listItem =
		'<a id="event-' +
		id +
		'" class="event-item-link" href="javascript:void(0);"' +
		"data-url='" +
		event.perma_link +
		"' data-image='" +
		itemImg +
		...
		'<span class="event-li-title event-detail">' +
		'<i class="svg-inline--fa fas fa-marker fa-read-me" data-toggle="tooltip" title="" data-placement="top"></i> ' +
		itemTitle +
		'</span>' +
		dateDisplayRow +
		itemLocation +
	...
	setTimeout(function () {
		...
		var markerIcon = L.divIcon({
			html:
				...
				'src="' +
				faPath +
				'book-reader.svg" class="fa-svg fa-leaflet-map-marker">',
			iconSize: [24, 24],
			popupAnchor: [0, 3],
			...
			setTimeout(function () {
				if (locations.length !== 0) {
					...
					for (var i = 0; i < locations.length; i++) {
						...
						var text =
							placeName +
							locations[i].address.street +
							', <br>' +
							locations[i].address.zipcode +
							', ' +
							locations[i].city;
