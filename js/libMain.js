...
function generateAccessibilityTextBlock(serviceName) {
	var blockItem =
		'<span class="accessibility-text-block" role="img" alt="' + serviceName + '">' + serviceName + ' </span>';
	...
	if (description.indexOf('<a href=') !== -1) {
		...
		while (reFindLinksExec != null) {
			...
			if (reFindLinksExec[0].indexOf('webropol') !== -1) {
				// Find url
				var urlOfLink = new RegExp(/"(http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(\/\S*)?"/g).exec(
					reFindLinksExec[0]
				);
				// Generate iframe
				var iframeCode =
					'<iframe style="overflow-x: hidden;" frameborder="0" height="800px" scrolling="yes" src=' +
					urlOfLink[0] +
					' width="100%"></iframe>';
				// Push to array
			...
			// Normal links
				...
				if (reFindLinksExec[0].indexOf('@') === -1) {
					linksToReplace.push({ position: reFindLinksExec[0], replacement: generateExternalLink(reFindLinksExec[0]) });
				}
			}
			// Loop all links.
...
function addItem(item, listElement) {
	...
	if (listElement == 'accessibilityCelia') {
		...
		if (
			standardName.toLowerCase().indexOf('registeration') > -1 ||
		...
		if (isValue(item.website)) {
			...
			celiaDescription =
				celiaDescription +
				"<p class='service-info service-website' " +
				" aria-label='" +
				i18n.get('Website') +
				"'>" +
				"<i aria-hidden='true' class='fas fa-globe'></i><span class='sr-only'>" +
				i18n.get('Website') +
				":</span> <a target='_blank' href='" +
				item.website +
				"'>" +
				capitalize(prettyLink) +
				'</a></p>';
		}
		if ($('.celia-services').length) {
			if ($('.celia-services').attr('data-content') != celiaDescription) {
				...
				if (
					standardName.toLowerCase().indexOf('registration') > -1 ||
					standardName.toLowerCase().indexOf('rekisteröityminen') > -1
				) {
	...
	if (listElement === '#accessibilityItems' && accessibilityIsEmpty) {
		...
		if (
			isValue(item.shortDescription) ||
			isValue(item.description) ||
			isValue(item.website) ||
			isValue(item.email) ||
			isValue(item.phoneNumber)
		) {
			...
			if (isValue(item.price)) {
				...
				description =
					description +
					'<p class="service-info service-price" aria-label="' +
					i18n.get('Price') +
					'">' +
					'<i class="fas fa-money-bill-alt" data-toggle="tooltip" title="' +
					i18n.get('Price') +
			...
			if (isValue(item.website)) {
				...
				description =
					description +
					'<p class="service-info service-website" aria-label="' +
					i18n.get('Website') +
					'"><i class="fas fa-globe" data-toggle="tooltip" title="' +
					i18n.get('Website') +
					'" ' +
					'data-placement="top"></i><a target="_blank" href="' +
					item.website +
					'">' +
					capitalize(prettyLink) +
					'</a></p>';
			...
			if (isValue(item.email)) {
				...
				description =
					description +
					'<p class="service-info service-email" aria-label="' +
					i18n.get('Email') +
					'">' +
					'<i class="fas fa-envelope-square" data-toggle="tooltip" title="' +
					i18n.get('Email') +
					'" ' +
					'data-placement="top"></i>' +
					mailToLink +
			...
			if (isValue(item.phoneNumber)) {
				description =
					description +
					'<p class="service-info service-phone" aria-label="' +
					i18n.get('Phone') +
					'">' +
					'<i class="fas fa-phone-square" data-toggle="tooltip" title="' +
					i18n.get('Phone') +
					'" ' +
					'data-placement="top"></i>' +
			...
			if (description.indexOf('<a href=') !== -1) {
				...
				while (reFindLinksExec != null) {
					...
					if (reFindLinksExec[0].indexOf('embed') !== -1) {
						...
						// Generate iframe
						var iframeCode =
							'<iframe frameborder="0" height="500px" scrolling="no" src=' + urlOfLink[0] + ' width="100%"></iframe>';
						// Push to array
						linksToReplace.push({ position: reFindLinksExec[0], replacement: iframeCode });
					}
					// Microsoft Bookings calendars
						...
						// Find url
						var BookingUrlOfLink = new RegExp(
							/"(http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(\/\S*)?"/g
						).exec(reFindLinksExec[0]);
						// Generate iframe if not null (Link invalid)
						if (BookingUrlOfLink !== null) {
							var iframeCode =
								'<iframe frameborder="0" height="1550px" scrolling="yes"  src=' +
								BookingUrlOfLink[0] +
					...
					// Normal links
						...
						// Push to array
						linksToReplace.push({
							position: reFindLinksExec[0],
							replacement: reFindLinksExec[0].replace(
								/(<a href=")+/g,
...
function bindActions() {
	...
	if (activeTab === 0) {
		$.when(navigateToDefault(0)).then(function () {
			//adjustParentHeight();
		});
	}
