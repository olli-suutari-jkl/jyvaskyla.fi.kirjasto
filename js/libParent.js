...
window.addEventListener('message', function (event) {
	...
	if (data.type === 'libList') {
		...
		if (refParamCount == 2) {
			...
			if (
				serviceNameInUrl != '?yhteystiedot' &&
				serviceNameInUrl != '?contacts' &&
				serviceNameInUrl != fiLibNameInUrl &&
				serviceNameInUrl != enLibNameInUrl
			) {
		...
		if (lang === 'fi') {
			if (enLibNameInUrl != '') {
				for (var i = 0; i < libList.length; i++) {
					if (
						enLibNameInUrl.indexOf(libList[i].nameEn) > -1 &&
						libList[i].id != currentLib &&
			...
			if (fiLibNameInUrl != '') {
				for (var i = 0; i < libList.length; i++) {
					if (
						fiLibNameInUrl.indexOf(libList[i].nameFi) > -1 &&
						libList[i].id != currentLib &&
						libList[i].nameEn !== libList[i].nameFi
					) {
