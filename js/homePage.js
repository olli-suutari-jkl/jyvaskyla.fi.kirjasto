	...
	for (var i = 0; i < items.length; i++) {
		...
		if (holidayDescription !== undefined) {
			...
			$('#specialInfo').replaceWith(
				'<span id="specialInfo" class="info-span info-text"><i class="fas fa-info-circle" > </i> ' +
					holidayDescription +
					'</span>'
			);
		} else {
			$('#specialInfo').replaceWith(
				'<span id="specialInfo" style="display: none" class="info-span info-text"><i class="fas fa-info-circle" > </i></span>'
			);
		}
		if (genericDescription !== undefined) {
			...
			$('#scheduleInfo').replaceWith(
				'<span id="scheduleInfo" class="info-span info-text"><i class="fas fa-info-circle" > </i> ' +
					genericDescription +
					'</span>'
			);
		} else {
			$('#scheduleInfo').replaceWith(
				'<span id="scheduleInfo" style="display: none" class="info-span info-text"><i class="fas fa-info-circle" > </i></span>'
			);
		}
	}
}
	...
	$.getJSON(
		...
		function (data) {
			if (data.items.length === 0) {
				...
				$('#dayInfo').replaceWith(
					'<span id="dayInfo" style="display: none" class="info-text"><i class="fas fa-info-circle" > </i></span>'
				);
				$('#scheduleInfo').replaceWith(
					'<span id="scheduleInfo" class="info-span info-text"><i class="fas fa-info-circle" > </i> ' +
						i18n.get('No opening hours') +
						'</span>'
				);
				return;
			}
			...
			for (var i = 0; i < schedules.length; i++) {
				...
				if (schedules != null) {
					...
					if (schedules[i].info !== null) {
						...
						$('#dayInfo').replaceWith(
							'<span id="dayInfo" class="info-span info-text"><i class="fas fa-info-circle"> </i> ' +
								infoText +
								'</span>'
						);
					...
					for (var t = 0; t < schedules[i].times.length; t++) {
						...
						if (time.status === 1) {
							...
							staffToday =
								'<tr class="time--sub time isTodayClass time--with-staff">' +
								'<td class="align-right trn"><i class="fas fa-level-up-alt fa-rotate-90"></i> ' +
						...
						// self-service
							...
							if (staffPresentStart === '') {
								selfServiceBefore =
									'<tr class="time--sub time isTodayClass time--no-staff">' +
									'<td class="align-right"><i class="fas fa-level-up-alt fa-rotate-90"></i> ' +
				...
				if (isClosed) {
					...
					scheludeRow =
						...
						isTodayClass +
						'">' +
						'<td colspan="2" class="main-schedule day-main-schedule closed">' +
						i18n.get('Closed') +
						'</td>' +
						'</tr>';
				} else {
					var mainSchedule =
						...
						isTodayClass +
						'">' +
						'<td colspan="2" class="main-schedule day-main-schedule">' +
						mainScheduleText +
						': <time datetime="' +
						dayStart +
						'">' +
					...
					if (staffToday.length !== 0 && (selfServiceBefore.length !== 0 || selfServiceAfter.length !== 0)) {
						mainSchedule = '';
					}
