	...
	setTimeout(function () {
		...
		if (!matchFound) {
			...
			for (var i = 0; i < libListMultiLang.length; i++) {
				if (
					urlUnescapeSpaces.indexOf(libListMultiLang[i].nameFi) > -1 ||
		...
		if (!matchFound) {
			if (refUrl.indexOf('halssila') > -1) {
				...
				refUrl.indexOf('saynat') > -1 ||
				refUrl.indexOf('s%c3%a4yn%c3%a4t') > -1 ||
				refUrl.indexOf('s%C3%A4yn%C3%A4tsalo') > -1 ||
				refUrl.indexOf('säynät') > -1
			) {
				library = 85117;
				if (lang === 'fi') {
